...
actionDef : '(' ':action' actionSymbol parametersDef? actionDefBody ')'
       -> ^(ACTION actionSymbol parametersDef? actionDefBody)
    ;

parametersDef : ':parameters' '(' typedVariableList ')'
	-> ^(ACTION_PARAMS_LIST typedVariableList)
	;
...
atomicTermFormula
	...
	| '(' '=' term term ')' -> ^(PRED_HEAD EQ_PRED term term)
	| '(' 'at' term+ ')' -> ^(PRED_HEAD 'at' term+)
	;
...
timedGD
	: '(' timeSpecifier goalDesc ')'
	| '(' interval goalDesc ')'
	;

timeSpecifier : AT_START | AT_END ;
interval : 'over-all' ;
...
simpleDurationConstraint
	...
	| '(' timeSpecifier simpleDurationConstraint ')'
	;
...
timedEffect
	: '(' timeSpecifier daEffect ')'     // BNF has a-effect here, but not defined anywhere
	| '(' timeSpecifier fAssignDA ')'
